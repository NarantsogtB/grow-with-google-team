...
def create_new_hospital(
    db: Database, hospital_data: HospitalCreate
) -> HospitalResponse:
    logger.info(f"{hospital_data.hospital_name}-тэй эмнэлэг үүсгэх хүсэлт ирлээ")
    existing_hospital = (
        db.query(Hospital)
        .where(Hospital.hospital_phone == hospital_data.hospital_phone)
        .first()
    )

    if existing_hospital:
        logger.warning(
            f"{hospital_data.hospital_phone} дугаартай эмнэлэг хэдийн үүссэн байна."
        )
